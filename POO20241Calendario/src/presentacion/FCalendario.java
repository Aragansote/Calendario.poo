...
public class FCalendario extends JFrame implements ActionListener {
	...
	public void mostrarCalendario() {
	    ...
	    int totalColumnas = 7 - diaDeSemana + ultimoDia;
	    int totalFilas = (totalColumnas + 6) / 7; 
	    this.pMes.setLayout(new GridLayout(totalFilas, 7));
	    ...
	    for (int i = 0; i < totalFilas; i++) {
	        for (int j = 0; j < 7; j++) {
	            if (i == 0 && j < diaDeSemana) {
	                ...
	                this.bDias[i][j].setVisible(true); 
	                dia++;
	            } else {
	                this.bDias[i][j].setText("");
	                this.bDias[i][j].setVisible(false); 
